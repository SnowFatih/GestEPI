    ...
    <Component
      ...
      className={cs(
        ...
        [
          fullWidth && "w-full",
          color === "primary" &&
            "bg-emerald-400 hover:bg-emerald-500 text-slate-50",
          color === "secondary" &&
            "bg-slate-700 hover:bg-slate-800 text-slate-50",
          color === "neutral" &&
            "bg-slate-200 hover:bg-slate-300 text-slate-50",
          color === "alert" && "bg-rose-500 hover:bg-rose-600 text-slate-50",
          color === "white" && "bg-white hover:bg-slate-300 text-slate-700",
          color === "grey" && "bg-slate-300 hover:bg-slate-400 text-black",
          marginClass && marginClass,
          disabled && "opacity-50 cursor-not-allowed",
          variant === "outlined" &&
            "border border-slate-300 border-dashed bg-transparent",
          variant === "outlined" &&
            color === "primary" &&
            "!text-emerald-400 border-emerald-400 hover:border-emerald-500 hover:text-emerald-500",
          variant === "outlined" &&
            color === "secondary" &&
            "!text-slate-700 border-slate-700 hover:border-slate-800 hover:text-slate-800",
          variant === "outlined" &&
            color === "neutral" &&
            "!text-slate-200 border-slate-200 hover:border-slate-300 hover:text-slate-300",
          variant === "outlined" &&
            color === "alert" &&
